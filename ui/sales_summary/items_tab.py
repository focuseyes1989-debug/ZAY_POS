...
class ItemsTab(QWidget):
    ...
    def _display_data(self, rows):
        ...
        for row_data in rows:
            ...
            self.table.setItem(r, 2, QTableWidgetItem(str(row_data[2])))
            self.table.setItem(r, 3, QTableWidgetItem(format_money(row_data[3], symbol)))
            self.table.setItem(r, 4, QTableWidgetItem(format_money(row_data[4], symbol)))
            self.table.setItem(r, 5, QTableWidgetItem(format_money(row_data[5], symbol)))
    ...
    def retranslateUi(self):
        lang_code = self.parent_page.get_lang() if self.parent_page else "en"
        if lang_code == "my":
            self.search_input.setPlaceholderText("ပစ္စည်းအမည်ရှာရန်...")
